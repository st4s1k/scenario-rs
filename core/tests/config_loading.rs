...
fn load_child_with_parent_inheritance() {
    ...
    // Child overrides parent server host
    assert_eq!(config.server.host, "service-one.example.com");
    // Child overrides parent server port
    assert_eq!(config.server.port, Some(23));
    // Child overrides parent credentials username
...
fn circular_parent_returns_error() {
    ...
    // a → b → a (use forward slashes to avoid TOML escape issues on Windows)
